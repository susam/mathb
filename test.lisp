...
(test-case slug-to-path
  (assert (string= (slug-to-path "/x/" "1") "/x/0/0/1.md"))
  (assert (string= (slug-to-path "/x/" "12") "/x/0/0/12.md"))
  (assert (string= (slug-to-path "/x/" "123") "/x/0/0/123.md"))
  (assert (string= (slug-to-path "/x/" "1234") "/x/0/1/1234.md"))
  (assert (string= (slug-to-path "/x/" "12345") "/x/0/12/12345.md"))
  (assert (string= (slug-to-path "/x/" "123456") "/x/0/123/123456.md"))
  (assert (string= (slug-to-path "/x/" "1234567") "/x/1/1234/1234567.md"))
  (assert (string= (slug-to-path "/x/" "12345678") "/x/12/12345/12345678.md")))
...
(test-case dodgy-content-p
  ...
  (assert (not (dodgy-content-p '(:words ("quux")) "foo" "bar" "qux")))
  (assert (not (dodgy-content-p '(:words ("bar")) "b" "a" "r")))
  (assert (dodgy-content-p '(:words ("foo")) "foo" "bar" "qux"))
  (assert (dodgy-content-p '(:words ("bar")) "foo" "bar" "qux"))
  (assert (dodgy-content-p '(:words ("qux")) "foo" "bar" "qux"))
  (assert (dodgy-content-p '(:words ("bar")) "foobarqux" "" ""))
  (assert (dodgy-content-p '(:words ("bar")) "" "foobarqux" ""))
  (assert (dodgy-content-p '(:words ("bar")) "" "" "foobarqux"))
  (assert (dodgy-content-p '(:words ("bar")) "" "" "foobarqux"))
  (assert (dodgy-content-p '(:words ("foo" "bar" "baz")) "foo" "" ""))
  (assert (dodgy-content-p '(:words ("foo" "bar" "baz")) "" "bar" ""))
  (assert (dodgy-content-p '(:words ("foo" "bar" "baz")) "" "" "baz"))
  (assert (dodgy-content-p '(:words ("foo" "bar" "baz")) "foobarbaz" "" ""))
  (assert (dodgy-content-p '(:words ("foo" "bar" "baz")) "" "foobarbaz" ""))
  (assert (dodgy-content-p '(:words ("foo" "bar" "baz")) "" "" "foobarbaz")))
...
(test-case reject-post-p-not
  ...
  (let ((x (write-to-string (calc-token 123))))
    ...
    (assert (not (reject-post-p '(:words ("quux")) "ip1" 0 "foo" "bar" "baz" x)))))
...
(test-case reject-post-p-words
  (clrhash *flood-table*)
  (let ((x (write-to-string (calc-token 123))))
    (assert (string= (reject-post-p '(:words ("xy")) 0 "ip1" "xy" "yz" "zx" x)
                     "Dodgy content!"))))
